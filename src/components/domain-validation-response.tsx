...
export function DomainValidationResponse({ domain }: { domain: DomainResponse | null }) {
    // Domain is not eligible
    if (!domain?.isEligible) {
        return (
            <>
                The email domain {domain ? domain.domain : ""} is not eligible for ACCESS. Please try again with your university or work email address.
            </>
        );
    }
